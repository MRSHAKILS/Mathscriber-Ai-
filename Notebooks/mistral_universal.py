#!/usr/bin/env python3
"""
Universal Image to LaTeX Converter - Mistral Vision API Version
Converts any image (equations, tables, diagrams) into complete LaTeX documents.
Uses Mistral's Pixtral vision model with intelligent content detection.
"""
...
def create_universal_latex(image_path: str, content_info: dict, mode: str = "mathjax") -> str:
    """
    Generates LaTeX code for any content type using Mistral Vision API.
    ...
    print("=" * 70)
    print("MISTRAL UNIVERSAL LATEX CONVERTER")
    print("=" * 70)
...
Generate clean LaTeX code for MathJax now:"""
    ...
    try:
        ...
        print(f"Error generating LaTeX with Mistral: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python mistral_universal.py <image_path> [output_path] [mode]")
        print("Example: python mistral_universal.py myimage.png output.tex")
        print("         python mistral_universal.py myimage.png output.tex compiler")
        print("\nModes:")
    ...
    if len(sys.argv) >= 3:
        ...
        output_path = test_outputs_dir / f"{Path(image_path).stem}_mistral_universal.tex"
