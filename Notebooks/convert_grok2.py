...
def create_latex_standalone(image_path: str) -> str:
    """
    Generates a complete, standalone, and compilable LaTeX document using Grok 2.
    Auto-detects whether the image contains a table or equation and processes accordingly.
    """
    ...
    # Universal prompt that handles both tables and equations - generates only code without document wrapper
    prompt_text = """You are a precision LaTeX code generator. Convert the provided image to LaTeX code.
...
4. FOR EQUATIONS:
   - For single numbered equation: \\begin{equation} ... \\end{equation}
   - For single unnumbered equation: \\begin{equation*} ... \\end{equation*}
   - For multiple aligned equations: \\begin{align} ... \\end{align}
   - For multiple unnumbered equations: \\begin{align*} ... \\end{align*}
   - Use \\frac{numerator}{denominator} for fractions
   - Use \\sqrt{} or \\sqrt[n]{} for roots
   - Use ^{} for superscripts and _{} for subscripts
   - Use \\sum, \\int, \\prod, \\lim with proper limits
...
Generate the clean LaTeX code now:"""
    ...
    print("Generating LaTeX document using Grok 2 API...")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python convert_grok2.py <path_to_image>")
        sys.exit(1)
    ...
    output_filename = f"{input_name}_grok2.tex"
