...
def process_image_to_latex(image_path, task_type):
    ...
    # Handle Grok 2 processing separately (handles both tables and equations)
    if task_type == 'grok2':
        return process_with_grok2(image_path)
    ...
    # Handle DeepSeek diagram processing separately
    if task_type == 'deepseek_diagram':
        return process_with_deepseek_diagram(image_path)
    
    # Handle DeepSeek AI diagram processing
    if task_type == 'deepseek_diagram_ai':
        return process_with_deepseek_diagram_ai(image_path)
    
    # Handle HuggingFace intelligent diagram generation
    if task_type == 'huggingface_diagram':
        return process_with_huggingface_diagram(image_path)
    
    # Handle OpenAI diagram processing separately
    if task_type == 'openai_diagram':
        return process_with_openai_diagram(image_path)
...
def process_with_grok2(image_path):
    """
    Process image (table or equation) using Groq API via convert_grok2.py script.
    Returns LaTeX output as a string.
    """
    try:
        # Path to convert_grok2.py in Notebooks directory
        notebooks_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'Notebooks')
        convert_script = os.path.join(notebooks_dir, 'convert_grok2.py')
        
        if not os.path.exists(convert_script):
            return f"% Error: convert_grok2.py not found at {convert_script}"
        
        print(f"[INFO] Processing with Grok 2 API: {image_path}")
        ...
        if result.returncode != 0:
            error_msg = result.stderr if result.stderr else "Unknown error"
            print(f"[ERROR] Error running convert_grok2.py: {error_msg}")
            return f"% Error running Grok 2 conversion: {error_msg}"
        
        # The script creates a .tex file with the same base name as the image
        base_name = os.path.splitext(os.path.basename(image_path))[0]
        tex_output_file = f"{base_name}_grok2.tex"
        ...
        if os.path.exists(tex_output_file):
            ...
            print(f"[SUCCESS] Grok 2 conversion successful")
            return latex_content
        else:
            # Try to find the file in the current directory or with different naming
            print(f"[WARNING] Expected output file {tex_output_file} not found")
        ...
        print("[ERROR] Grok 2 API call timed out")
        return "% Error: Grok 2 API call timed out (exceeded 2 minutes)"
    except Exception as e:
        print(f"[ERROR] Error processing with Grok 2: {e}")
        return f"% Error processing with Grok 2: {str(e)}"
...
def process_with_deepseek_diagram(image_path):
    """
    Process diagram image using DeepSeek API via deepseek_diagram.py script.
    Returns LaTeX TikZ code as a string.
    """
    try:
        # Path to deepseek_diagram.py in Notebooks directory
        notebooks_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'Notebooks')
        convert_script = os.path.join(notebooks_dir, 'deepseek_diagram.py')
        
        if not os.path.exists(convert_script):
            return f"% Error: deepseek_diagram.py not found at {convert_script}"
        
        print(f"[INFO] Processing diagram with DeepSeek API: {image_path}")
        ...
        # Run deepseek_diagram.py script
        result = subprocess.run(
            [sys.executable, convert_script, image_path],
            capture_output=True,
            text=True,
        ...
        if result.returncode != 0:
            error_msg = result.stderr if result.stderr else "Unknown error"
            print(f"[ERROR] Error running deepseek_diagram.py: {error_msg}")
            return f"% Error running DeepSeek diagram conversion: {error_msg}"
        
        # The script creates a .tex file with the same base name as the image
        base_name = os.path.splitext(os.path.basename(image_path))[0]
        tex_output_file = f"{base_name}_diagram.tex"
        ...
        if os.path.exists(tex_output_file):
            ...
            print(f"[SUCCESS] DeepSeek diagram conversion successful")
            return latex_content
        else:
            # Try to find the file in the current directory or with different naming
            print(f"[WARNING] Expected output file {tex_output_file} not found")
        ...
        print("[ERROR] DeepSeek API call timed out")
        return "% Error: DeepSeek API call timed out (exceeded 2 minutes)"
    except Exception as e:
        print(f"[ERROR] Error processing diagram with DeepSeek: {e}")
        return f"% Error processing diagram with DeepSeek: {str(e)}"

def process_with_openai_diagram(image_path):
    """
    Process diagram image using OpenAI API via openai_diagram.py script.
    Returns LaTeX TikZ code as a string.
    """
    try:
        # Path to openai_diagram.py in Notebooks directory
        notebooks_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'Notebooks')
        convert_script = os.path.join(notebooks_dir, 'openai_diagram.py')
        
        if not os.path.exists(convert_script):
            return f"% Error: openai_diagram.py not found at {convert_script}"
        
        print(f"[INFO] Processing diagram with OpenAI API: {image_path}")
        ...
        if result.returncode != 0:
            error_msg = result.stderr if result.stderr else "Unknown error"
            print(f"[ERROR] Error running openai_diagram.py: {error_msg}")
            return f"% Error running OpenAI diagram conversion: {error_msg}"
        
        # The script creates a .tex file with the same base name as the image
        base_name = os.path.splitext(os.path.basename(image_path))[0]
        tex_output_file = f"{base_name}.tex"
        ...
        if os.path.exists(tex_output_file):
            ...
            print(f"[SUCCESS] OpenAI diagram conversion successful")
            return latex_content
        else:
            # Try to find the file in the current directory or with different naming
            print(f"[WARNING] Expected output file {tex_output_file} not found")
        ...
        print("[ERROR] OpenAI API call timed out")
        return "% Error: OpenAI API call timed out (exceeded 2 minutes)"
    except Exception as e:
        print(f"[ERROR] Error processing diagram with OpenAI: {e}")
        return f"% Error processing diagram with OpenAI: {str(e)}"
...
def process_with_deepseek_diagram_ai(image_path):
    """Process diagram using DeepSeek AI with intelligent fallback"""
    print("=" * 80)
    print("DEEPSEEK AI DIAGRAM CONVERTER")
    print("=" * 80)
    
    notebooks_dir = Path(__file__).parent.parent / 'Notebooks'
    script_path = notebooks_dir / 'deepseek_diagram_ai.py'
    
    if not script_path.exists():
        print(f"[ERROR] Script not found: {script_path}")
        return "% Error: deepseek_diagram_ai.py script not found"
    
    try:
        # Run the DeepSeek AI converter
        result = subprocess.run(
        ...
        print(f"[INFO] DeepSeek converter output:\n{result.stdout}")
        
        if result.stderr:
            print(f"[WARNING] Stderr: {result.stderr}")
        
        # Look for the generated .tex file
        image_stem = Path(image_path).stem
        tex_output_file = Path(image_path).parent / f"{image_stem}_deepseek_ai.tex"
        
        if tex_output_file.exists():
            with open(tex_output_file, 'r', encoding='utf-8') as f:
                latex_content = f.read()
            print(f"[SUCCESS] Read {len(latex_content)} characters from {tex_output_file}")
            return latex_content
        else:
            print(f"[WARNING] Expected output file {tex_output_file} not found")
            if result.stdout:
...
def process_with_huggingface_diagram(image_path):
    """Process diagram using HuggingFace intelligent analysis"""
    print("=" * 80)
    print("HUGGINGFACE INTELLIGENT DIAGRAM GENERATOR")
    print("=" * 80)
    
    notebooks_dir = Path(__file__).parent.parent / 'Notebooks'
    script_path = notebooks_dir / 'huggingface_diagram.py'
    
    if not script_path.exists():
        print(f"[ERROR] Script not found: {script_path}")
        return "% Error: huggingface_diagram.py script not found"
    
    try:
        # Run the HuggingFace intelligent generator
        result = subprocess.run(
            [sys.executable, str(script_path), image_path],
            capture_output=True,
            text=True,
            timeout=60,  # 1 minute timeout (faster than AI)
            encoding='utf-8',
            errors='replace'
        )
        
        print(f"[INFO] HuggingFace generator output:\n{result.stdout}")
        ...
        # Look for the generated .tex file
        image_stem = Path(image_path).stem
        tex_output_file = Path(image_path).parent / f"{image_stem}_huggingface.tex"
        ...
        print("[ERROR] HuggingFace generation timed out")
        return "% Error: HuggingFace generation timed out (exceeded 1 minute)"
    except Exception as e:
        print(f"[ERROR] Error processing diagram with HuggingFace: {e}")
        return f"% Error processing diagram with HuggingFace: {str(e)}"